...
module.exports.assignReview = async (req, res) => {
  ...
  try {
    if (req.isAuthenticated()) {
      ...
      const alreadyAssigned = reviewer.assignedUserReviews.filter(
      ...
      // update reviewer's assignedUserReviews field by putting reference of recipient
      await reviewer.updateOne({
        $push: { assignedUserReviews: recipient },
      });
...
module.exports.submitReview = async (req, res) => {
  ...
  try {
    ...
    await recipient.updateOne({
      $push: { reviewsRecievedFromOthers: review },
    });

    // remove reference of the recipient from the reviewer's assignedUserReviews field
    await reviewer.updateOne({
      $pull: { assignedUserReviews: recipient.id },
    });
